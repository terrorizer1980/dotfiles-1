" Lightline theme based on the original provided with the
" moonfly colorscheme (github.com/bluz71/vim-moonfly-colors)
...
if exists('g:lightline')
    ...
    let s:p.normal.middle   = [[s:white,   s:grey234]]
    let s:p.normal.right    = [[s:white,   s:grey236], [s:blue, s:grey234]]
    let s:p.normal.warning  = [[s:white,   s:grey236]]
    let s:p.normal.error    = [[s:blue,    s:grey236]]

    let s:p.inactive.left   = [[s:grey247, s:grey234], [s:grey247, s:grey234]]
    let s:p.inactive.midle = [[s:grey247, s:grey234]]
    let s:p.inactive.right  = [[s:grey247, s:grey234], [s:grey247, s:grey234]]
    ...
    let g:lightline#colorscheme#moonfly_ngs#palette = lightline#colorscheme#flatten(s:p)
endif
